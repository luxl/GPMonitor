...
bool Posix_QextSerialPort::open(OpenMode mode)
{
	...
	if (!isOpen()) {
		...
		if (Posix_File->open(QIODevice::ReadWrite|QIODevice::Unbuffered)) {
            ...
            Posix_CommConfig.c_lflag=0;//&=(~(ICANON|ECHO|ECHOE|ECHOK|ECHONL|ISIG));
            Posix_CommConfig.c_iflag=0;//&=(~(INPCK|IGNPAR|PARMRK|ISTRIP|ICRNL|IXANY));
            Posix_CommConfig.c_oflag=0;//&=(~OPOST);
