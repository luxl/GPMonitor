...
{
    return sendData(dataToSend, 1000);
}

uchar ManageSerialPort::sendData(QByteArray &dataToSend, int interval)
{
    ...
    threadSend->addDataToSend(dataToSend, interval); //Ajout des donnees a envoyer
    return 1;
}
...
void ThreadSend::addDataToSend(QByteArray &dataToAdd, int interval)
{
...
void ThreadReceive::run()
{
	...
	forever
	{
		...
		if (numBytes > 0)
		{
            ...
            numBytes = extSerialPort.read(data, numBytes);
